...
def get_neighbors(state):
    ...
    for obj in ['man', 'cabbage', 'goat', 'wolf']:
        if getattr(state, obj) == state.man:  # Nếu đối tượng đang ở cùng vị trí với người đàn ông
            new_location = Location.A if state.man == Location.B else Location.B  # Xác định vị trí mới
    ...
    return neighbors
# Tìm đường đi
path = depth_first_search(start_state, lambda state: state == goal_state, get_neighbors) # Call the function and assign the result to path
